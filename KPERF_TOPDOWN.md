...
`scripts/920b/run.sh` 固定使用 `VLLM_USE_V2_MODEL_RUNNER=1` 和 eager，按事件组重启服务并调用 `run_one.sh`、`parse_run.py` 与 `summary.py`。模型参数固定为输入7000、输出100、并发1、TP=1、BF16。解析时排除每阶段第一条 prefill，并排除 `update_states` 未与其余阶段对齐的尾部记录。
