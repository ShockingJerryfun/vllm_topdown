...
using perf_event_open(2) system calls, compatible with both aarch64 (Kunpeng)
and x86_64 (AMD).
...
def _perf_event_open(attr, pid, cpu, group_fd, flags):
    ...
    return _libc.syscall(
        _PERF_EVENT_OPEN_NR, ctypes.byref(attr), pid, cpu, group_fd, flags
    )
...
def _kperf_init():
    ...
    for idx, (etype, cfg) in enumerate(_KPERF_EVENT_SPECS):
        ...
        if rc < 0:
            ...
            print(
                f"[kperf] perf_event_open #{idx} (cfg=0x{cfg:04x}) failed: errno={err} "
                f"({errno.errorcode.get(err, 'unknown')})",
                flush=True,
            )
            if err in (errno.EACCES, errno.EPERM):
                print(
                    "[kperf]   -> perf_event_paranoid 过高? "
    ...
    if fds:
        ...
        _kperf_enabled = True
        print(
            f"[kperf] enabled: prefix={_KPERF_PREFIX}, "
            f"time_mode={_KPERF_TIME_MODE}, events={_KPERF_EVENT_NAMES}, fds={fds}",
            flush=True,
        )
    else:
        print(
            f"[kperf] init failed: all perf_event_open failed, "
            f"specs={_KPERF_EVENT_SPECS}",
            flush=True,
        )
